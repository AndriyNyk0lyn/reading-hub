...
export function SavedFeed() {
	...
	const [stateFilter, setStateFilter] = useState<ArticleStateFilterValue>("all")
	...
	const filteredArticles = useMemo(() => {
		return savedArticles.filter((article) => {
			const matchesState = stateFilter === "all" || article.state === stateFilter
	...
	return (
		<section className="space-y-6">
			<div className="flex flex-col gap-4 rounded-lg border bg-card px-4 py-4">
				<ArticleStateFilter value={stateFilter} onValueChange={setStateFilter} />
						...
						<Card key={article.id}>
							...
							<CardContent className="space-y-2 text-sm text-muted-foreground">
								<p className="line-clamp-3">
									{article.description || "This saved article does not include a summary."}
								</p>
							...
							<CardFooter className="flex flex-col gap-3 sm:flex-row sm:justify-between">
								...
								<Button size="sm" variant="destructive" onClick={() => removeArticle(article.id)}>
