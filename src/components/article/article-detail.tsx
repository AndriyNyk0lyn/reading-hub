...
import { removeSavedArticle, saveArticleForOffline } from "@/lib/saved-articles"
		...
		return (
			<div className="space-y-4 rounded-lg border bg-card px-6 py-8">
				...
				<div className="flex flex-col gap-2 sm:flex-row sm:items-center">
					<Button onClick={() => refetch()} disabled={!allowRemoteFetch || isFetching}>
		...
		<article className="space-y-6">
			<div className="space-y-3">
				<div className="flex flex-wrap items-center gap-3">
					...
					{savedArticle && <Badge variant="secondary">Saved for offline reading</Badge>}
				</div>
				<Title>{article.title}</Title>
				<p className="text-muted-foreground">
					By {article.author}
					{article.publishedAt ? ` · ${new Date(article.publishedAt).toLocaleDateString()}` : null}
				</p>
			...
			<div className="flex flex-col gap-3 sm:flex-row">
				<Button onClick={handleSaveToggle} disabled={isSaving} className="flex-1 sm:flex-none">
					{savedArticle ? (
						<>
			...
			<div className="rounded-lg border bg-card px-6 py-6">
				{article.body ? (
					...
					<p className="text-muted-foreground">
						This article body is not available yet. Open the original article to read it online.
					</p>
				)}
			</div>
		</article>
